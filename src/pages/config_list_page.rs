...
use crate::components::config_view::ConfigView;
...
impl Component for ConfigListPage {
    type Message = Msg;
    type Properties = ();
...
impl ConfigListPage {
    fn view_configs(&self, _ctx: &Context<Self>, data: &ListResult) -> Html {
        ...
        let list = tmp_items.into_iter()
            ...
            .map(|el| {
            html! {
                <ConfigView id={el.1.id} strs={el.1.strs.clone()} strids={el.1.strids.clone()} strstrs={el.1.strstrs.clone()} bools={el.1.bools.clone()} ids={el.1.ids.clone()} u64s={el.1.u64s.clone()}/>
            }
        });
        ...
        if !prev_page.is_none() {
            prev_page_htm = html! {
                <>
                    <a href={ "/config?filter=".to_owned() + &self.filter.to_string() + "&find=" + &self.find.to_string() + "&skip=" + &((prev_page.unwrap() - 1) * eff_limit).to_string() + "&limit=" + &eff_limit.to_string() } class="pagination-previous">{"Previous"}</a>
                </>
            }
        }
        ...
        if !next_page.is_none() {
            next_page_htm = html! {
                <>
                    <a href={ "/config?filter=".to_owned() + &self.filter.to_string() + "&find=" + &self.find.to_string() + "&skip=" + &((next_page.unwrap() - 1) * eff_limit).to_string() + "&limit=" + &eff_limit.to_string() } class="pagination-next">{"Next"}</a>
                </>
            }
        }
        ...
        if !prev_page.is_none() && mid_page > 1 {
            prev_page_htm2 = html! {
                <>
                    <li>
                        <a href={ "/config?filter=".to_owned() + &self.filter.to_string() + "&find=" + &self.find.to_string() + "&skip=" + &((mid_page - 2) * eff_limit).to_string() + "&limit=" + &eff_limit.to_string() } class="pagination-previous" aria-label={ "Goto page ".to_owned() + &(mid_page - 1).to_string() }>{ (mid_page - 1).to_string() }</a>
                    </li>
                </>
            }
        };
        ...
        if !next_page.is_none() && mid_page < last_page {
            next_page_htm2 = html! {
                <>
                    <li>
                        <a href={ "/config?filter=".to_owned() + &self.filter.to_string() + "&find=" + &self.find.to_string() + "&skip=" + &((mid_page) * eff_limit).to_string() + "&limit=" + &eff_limit.to_string() } class="pagination-previous" aria-label={ "Goto page ".to_owned() + &(mid_page + 1).to_string() }>{ (mid_page + 1).to_string() }</a>
                    </li>
                </>
            }
        };
        ...
        let first_page_htm2 = if mid_page > 2 {
            html! {
                <>
                    <li>
                      <a href={ "/config?filter=".to_owned() + &self.filter.to_string() + "&find=" + &self.find.to_string() + "&skip=0&limit=" + &eff_limit.to_string() } class="pagination-link" aria-label="Goto page 1">{"1"}</a>
                    </li>
        ...
        let last_page_htm2 = if (mid_page + 1) < last_page {
            html! {
                <>
                    ...
                    <li>
                      <a href={ "/config?filter=".to_owned() + &self.filter.to_string() + "&find=" + &self.find.to_string() + "&skip=" + &((last_page - 1) * eff_limit).to_string() + "&limit=" + &eff_limit.to_string() } class="pagination-link" aria-label={"Goto page ".to_owned() + &last_page.to_string()}>{ last_page.to_string() }</a>
                    </li>
                </>
            }
        } else {
            ...
            <>
                <nav class="level">
                      ...
                      <div class="level-left">
                        <div class="level-item">
                          <p class="subtitle is-5">
                            <strong>{ data.total_count.to_string() }</strong>{" configuration(s)"}
                          </p>
                        </div>
                        <div class="level-item">
                          <div class="field has-addons">
                            <p class="control has-icons-left">
                                <input class="input" name="find" type="text" placeholder="Find configuration"/>
                    ...
                    <div class="level-right">
                        <p class="level-item"><a href="/config" class={if self.filter == "" { "has-text-weight-bold" } else { "" }}>{"All"}</a></p>
                        <p class="level-item pl-2">
                            <button onclick={self.add_config()} class="button">
